  ...
  return (
    <div className="animate-in fade-in duration-500 max-w-5xl mx-auto">
      ...
      <div className="mb-10 p-8 rounded-2xl border border-slate-800 bg-gradient-to-b from-slate-900/50 to-[#0d0d0f]">
        <button
          onClick={() => navigate("/interviews")}
          className="text-slate-500 hover:text-slate-300 text-sm flex items-center gap-2 mb-6 transition-colors"
        >
          ...
          <div className="animate-in fade-in slide-in-from-bottom-2 duration-300">
            {loading ? (
              ...
              <div className="text-center py-12 text-slate-500 italic">
                No problems available for this track yet.
              </div>
            ) : (
              <>
          ...
          <div className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
            <div className="flex justify-between items-center mb-6">
              <p className="text-sm text-slate-500">
                Community insights and interview experiences.
              </p>
            ...
            {track.discussions.length === 0 ? (
              ...
              track.discussions.map((disc) => (
                <div
                  key={disc.id}
                  className="flex items-start gap-4 p-5 rounded-xl border border-slate-800 bg-[#0d0d0f] hover:bg-slate-900/50 transition-colors cursor-pointer group"
                >
                  <div className="mt-1">
                    {disc.isPinned ? (
                      <Pin size={18} className="text-amber-500" />
                    ) : (
                      <MessageSquare size={18} className="text-slate-600 group-hover:text-indigo-400" />
                    )}
                  </div>
